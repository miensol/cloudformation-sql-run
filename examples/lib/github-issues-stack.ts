...
import { Http, HttpCall } from "cloudformation-sql-run";
...
export class GithubIssuesStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
    ...
    // aws secretsmanager create-secret --secret-string PERSONAL_API_TOKEN --name cloudformation-sql-run-github-api-token
    const githubApiToken = secretmanager.Secret.fromSecretNameV2(this, 'github api secret', 'cloudformation-sql-run-github-api-token')
    const baseRequest: HttpCall = {
      method: 'post',
      headers: {
        Accept: 'application/vnd.github.v3+json'
      },
      url: 'https://github.com/miensol/cloudformation-sql-run/issues'
    }
